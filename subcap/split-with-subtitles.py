...
def split_media_with_subcaps(media_file, vtt_file, dest_dir):
    ...
    for idx, segment in enumerate(merged_caps):
        mf[int(segment["start_time"] * 1000) : int(segment["end_time"] * 1000)].export(
            f"{dest_dir}/{idx}.mp3", format="mp3", bitrate="16k"
        )
...
def generate_dataset(ds_name, splits_dir):
    ...
    ds = datasets.Dataset.from_dict(
        {"uuid": uuids, "audio": mp3_files, "sentence": sentences, "start": start_times, "end": end_times}
    )
...
if __name__ == "__main__":
    ...
    # Add the arguments
    parser.add_argument("--root-dir", type=str, required=True, help="Root directory to start search from.")
